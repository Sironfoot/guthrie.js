...
Router.prototype.mapRoute = function(route, routeParams) {
    ...
    if (controllerName) {
        controller = this._resolveController(controllerName);
    }
    ...
    this.app.all(route, function(req, res, next) {
        ...
        if (!controller) {
            controller = router._resolveController(req.params.controller);
        ...
        res.view = function(locals, callback) {
			var view = path.join(router.viewsDir, req.params.controller, actionName);
